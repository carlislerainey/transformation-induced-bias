...
simulate <- function(n_sims = 100) {
  f <- onset ~ warl + gdpenl + lpopl1 + 
	...
	m$x_hi <- m$x
	m$x_hi[, "gdpenl"] <- m$x[, "gdpenl"] + 4.5 # -> about sd(m$x[, "gdpenl"])
	fl$prob <- predict(m, newdata = fl, type = "response")
	fl$me <- dnorm(m$x%*%coef(m))*coef(m)[3]
	fl$fd <- pnorm(m$x_hi%*%coef(m)) - pnorm(m$x%*%coef(m))
	coefs <- matrix(NA, nrow = n_sims, ncol = length(coef(m)))
	probs <- mes <- fds <- matrix(NA, nrow = n_sims, ncol = nrow(fl))
	# set progress bar
	...
	for (i in 1:n_sims) {
		...
		fds[i, ] <- pnorm(m$x_hi%*%coef(mle_fit)) - pnorm(m$x%*%coef(mle_fit))
	...
	tau_bias_fd <- e_fd - fl$fd
	ci_tau_bias_fd <- (pnorm(m$x_hi%*%e_coef) - pnorm(m$x%*%e_coef)) - fl$fd
	ti_tau_bias_fd <- e_fd - (pnorm(m$x_hi%*%e_coef) - pnorm(m$x%*%e_coef))
...
df6 <- data.frame(true = sim$true_me,  
                  ...
                  type = "'Transformation-Induced'~tau*'-Bias'")
df7 <- data.frame(true = sim$true_fd, 
                  qi = "First~Difference",
                  bias = sim$tau_bias_fd, 
                  type = "'Total'~tau*'-Bias'")
df8 <- data.frame(true = sim$true_fd,  
                  qi = "First~Difference",
                  bias = sim$ci_tau_bias_fd, 
                  type = "'Coefficient-Induced'~tau*'-Bias'")
df9 <- data.frame(true = sim$true_fd,  
                  qi = "First~Difference",
...
gg1 <- ggplot(data = subset(df, qi == "First~Difference"), aes(x = true, y = bias)) + 
...
gg2 <- gg1 %+% subset(df, qi == "Marginal~Effect") + 
  labs(title = "Marginal Effect")

gg3 <- gg1 %+% subset(df, qi == "Predicted~Probability") + 
  labs(title = "Predicted Probability")


# combine plots
pdf("doc/figs/fl.pdf", width = 12, height = 8)
grid.arrange(gg3, gg2, gg1)
dev.off()
...
f <- onset ~ warl + gdpenl + lpopl1 + 
  lmtnest + ncontig + Oil + nwstate + 
  instab + polity2l + ethfrac + relfrac
m <- glm(f, fl, family = binomial, x = TRUE)
texreg::screenreg(m)
