...
Although methodologists tend to focus on estimating the model coefficients, substantive researchers tend to focus on a ``quantity of interest''. This quantity of interest is simply a \textit{transformation} $\tau$ of the model coefficients \citep{KingTomzWittenberg2000}. 
...
Fortunately, the invariance principle allows the researcher to calculate estimates of the quantities of interest from the coefficient estimates.
...
Methodological work tends to focus on estimating model coefficients well, while substantive research tends to focus on estimating quantities of interest. 
...
However, the researcher is not likely interested in $\log(\text{income})$ directly. Instead, she is likely interested in income itself. In particular, she might be interested in the median income among those with 20 years of education $\med(\text{income} | \text{education} = 20) = e^{\beta_{cons} + 20\beta_{edu}}$. 
...
Figure \ref{fig:bias-coef} shows the bias in the coefficients as the sample size increases. The left panel plots the bias in $\hat{\beta}_{cons}$ and the right panel plots and $\hat{\beta}_1$. 
...
The left panel plots the total bias, the middle panel plots the coefficient-induced bias, and the right panel plots the transformation-induced bias.
...
While the coefficient estimates are approximately unbiased for about $N = 250$, the estimates of the marginal effects retain substantial bias for this same sample size. 
