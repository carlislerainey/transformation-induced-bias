...
Although methodologists tend to focus on estimating the model coefficients, substantive researchers tend to focus on a ``quantity of interest.'' A quantity of interest is simply a \textit{transformation} $\tau$ of the model coefficients \citep{KingTomzWittenberg2000}. 
...
However, the researcher is not likely interested in $\log(\text{income})$ directly. Instead, she is likely interested in income itself. 
